      ...
      try {
        ...
        try {
          ...
          if (statsRes.ok) {
            ...
            byDate = statsData.by_date || {};
          }
        } catch (e) {
          console.warn("API analytics failed", e);
        }
        ...
        if (localMessages && localSessions) {
          ...
          let localLogs: ChatLog[] = [];
          for (const session of parsedSessions) {
            const msgs = parsedMsgs[session.id] || [];
            for (let i = 0; i < msgs.length; i++) {
              if (msgs[i].role === 'user') {
                const aiMsg = msgs[i+1]?.role === 'assistant' ? msgs[i+1] : null;
                const timestampStr = new Date(session.timestamp).toISOString();
                localLogs.push({
                  id: msgs[i].id,
                  session_id: session.id,
                  user_message: msgs[i].content,
                  ai_response: aiMsg ? aiMsg.content : '',
          ...
          if (localLogs.length > 0) {
             ...
             const newLocalLogs = localLogs.filter(l => !existingIds.has(l.id));
             allLogs = [...newLocalLogs, ...allLogs].slice(0, 100);
             
             totalInteractions += newLocalLogs.length;
        ...
        setLogs(allLogs);
        setStats({ total_interactions: totalInteractions, by_date: byDate });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
        <div className="p-5 bg-rose-50 dark:bg-rose-500/10 rounded-xl border border-rose-100 dark:border-rose-500/20 text-center">
          <p className="text-sm font-medium text-rose-600 dark:text-rose-400 mb-1">{t('admin.totalInteractions', 'Tổng lượt hội thoại')}</p>
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats?.total_interactions || 0}</p>
        </div>
        <div className="p-5 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl border border-indigo-100 dark:border-indigo-500/20 text-center">
          <p className="text-sm font-medium text-indigo-600 dark:text-indigo-400 mb-1">{t('admin.today', 'Hôm nay')}</p>
      ...
      {/* Logs Table */}
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{t('admin.recentChats', 'Lịch sử trò chuyện gần đây')}</h3>
        ...
        {logs.length === 0 ? (
          <div className="text-center py-10 text-gray-500 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-gray-200 dark:border-slate-700">
            {t('admin.noChats', 'Chưa có lịch sử hội thoại nào')}
          </div>
        ) : (
            ...
            {logs.map((log) => (
              <div key={log.id} className="p-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-gray-100 dark:border-slate-700/50 hover:border-rose-200 dark:hover:border-rose-700/50 transition-colors">
                <div className="flex items-center gap-2 mb-3 text-xs text-gray-400">
                  ...
                  {new Date(log.timestamp).toLocaleString(i18n.language === 'vi' ? 'vi-VN' : 'en-US')}
                </div>
