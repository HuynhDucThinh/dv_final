...
def _make_provider_llm(
    ...
    # --- Providers mới: Groq và OpenAI (THÊM MỚI cho module Phân tích Dữ liệu) ---
    if provider_id == GROQ_PROVIDER:
        resolved_api_key = (api_key or GROQ_API_KEY or "").strip()
        if not resolved_api_key:
            logger.warning("Groq API key không được cấu hình; bỏ qua model %s.", model_name)
            return None
        return _make_openai_chat(
            model_name=model_name,
            api_key=resolved_api_key,
            base_url=PROVIDER_REGISTRY[GROQ_PROVIDER].base_url,
            temperature=temperature,
            max_tokens=max_tokens,
            timeout=timeout,
        )
    ...
    if provider_id == OPENAI_PROVIDER:
        ...
        if not resolved_api_key:
            logger.warning("OpenAI API key không được cấu hình; bỏ qua model %s.", model_name)
            return None
        return _make_openai_chat(
            model_name=model_name,
            api_key=resolved_api_key,
            base_url=PROVIDER_REGISTRY[OPENAI_PROVIDER].base_url,
            temperature=temperature,
            max_tokens=max_tokens,
            timeout=timeout,
        )
